...
class Link:private juce::Thread {
 Port port;uint32_t sequence=0;juce::CriticalSection lock;juce::String path,message="Disconnected";bool desired=false,readPending=false;int action=0;std::map<int,int> writes,received;bool snapshot=false;
 ...
 bool want,read;int command;juce::String selected;std::map<int,int> pending;{const juce::ScopedLock g(lock);want=desired;command=action;action=0;selected=path;read=readPending;readPending=false;pending.swap(writes);}
 ...
 if(read)readControls();if(command)request(command);for(auto& item:pending){int v=item.second;if(item.first==20)v=v?65535:0;request(3,{uint8_t(item.first),uint8_t(item.first>>8),uint8_t(v),uint8_t(v>>8)});}
 auto s=request(2);{const juce::ScopedLock g(lock);message=juce::String(word(s,30)?"Connected / LINE-IN ON / losses ":"Connected / STOPPED / losses ")+juce::String(dword(s,8))+" / errors "+juce::String(dword(s,20));}wait(50);
...
public:
 ...
 void disconnect(){const juce::ScopedLock g(lock);desired=false;action=0;writes.clear();message="Disconnected";notify();}
